...
export default function Hero() {
  ...
  return (
    ...
    >
      <div className="halftone pointer-events-none absolute inset-0 -z-10 opacity-60 [mask-image:radial-gradient(ellipse_at_top,black_10%,transparent_70%)]" />
      <div className="pointer-events-none absolute -left-16 top-10 -z-10 h-64 w-64 rounded-full bg-primary/25 blur-[90px]" />
      <div className="pointer-events-none absolute -right-16 top-72 -z-10 h-56 w-56 rounded-full bg-brand-sky/20 blur-[80px]" />

      {/* Floating game pieces */}
      <div className="hero-piece hero-piece-1 absolute right-8 top-20 flex h-12 w-12 items-center justify-center rounded-2xl border-2 border-black bg-primary shadow-lg shadow-primary/30">
        <Dices className="h-6 w-6 text-primary-foreground" />
      </div>
      <div className="hero-piece hero-piece-2 absolute right-4 top-[19rem] flex h-10 w-10 items-center justify-center rounded-full border-2 border-black bg-accent shadow-lg shadow-accent/30">
        <Sparkles className="h-5 w-5 text-accent-foreground" />
      </div>
      <div className="hero-piece hero-piece-3 absolute left-3 top-4 flex h-11 w-11 items-center justify-center rounded-xl border-2 border-black bg-brand-sky shadow-lg">
        <Gamepad2 className="h-5 w-5 text-brand-sky-foreground" />
      </div>

      <div className="hero-badge mb-8 inline-flex items-center gap-2 rounded-full border-2 border-accent bg-accent/10 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-accent">
        <Dices className="h-3.5 w-3.5" />
        Egypt&apos;s First Board Games Cafe
      </div>

      <h1 className="mb-6 text-6xl font-headline font-extrabold uppercase leading-[0.95] tracking-tight">
        {HEADLINE_LINES.map((line, i) => (
          <span key={line} className="block overflow-hidden pb-1">
            <span
              className={`hero-line block ${i === 1 ? "text-pop text-accent" : ""}`}
            >
              {line}
            </span>
          </span>
